...
/* Return index of best performing particle */
int pso_omp_best_fitness(swarm_t *swarm, int num_threads)
{
    int i, g;
    float best_fitness = INFINITY;
    particle_t *particle;
    ...
    #pragma omp parallel for default(none) private(i, particle) shared(swarm, best_fitness, g)
    for (i = 0; i < swarm->num_particles; i++) {
        particle = &swarm->particle[i];
        #pragma omp critical
        {
...
/* Parallelized PSO Initialization */
swarm_t *pso_omp_init(char *function, int dim, int swarm_size, 
                  float xmin, float xmax, int num_threads)
{
    ...
    #pragma omp parallel for default(none) private(i, j, particle, status, fitness) shared(dim, xmax, xmin, swarm, function, stderr)
    for (i = 0; i < swarm->num_particles; i++) {
        particle = &swarm->particle[i];
        particle->dim = dim; 
        /* Generate random particle position */
